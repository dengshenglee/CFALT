...
class ExpListNode extends ASTnode {
    ...
    public void typeCheck(List<Type> types) {
        if (myExps.size() == types.size()) {
            for (int i = 0; i < myExps.size(); i++) {
                Type myType = myExps.get(i).typeCheck();
                if (myType.isErrorType()) {
                    continue;
                }
                if (!myType.equals(types.get(i))) {
                    ErrMsg.fatal(myExps.get(i).lineNum(),
                    myExps.get(i).charNum(),
...
class PostIncStmtNode extends StmtNode {
    ...
    public void typeCheck(Type type) {
        ...
        if (tempType.isErrorType() || tempType.isIntType()) {
            return;
        }
...
class ReadStmtNode extends StmtNode {
    ...
    public void typeCheck(Type type) {
        ...
        else if (tempType.isStructDefType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
                    "Attempt to read a struct name");
        }
        else if (tempType.isFnType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
...
class IfStmtNode extends StmtNode {
    ...
    public void typeCheck(Type type) {
        Type tempType = myExp.typeCheck();
        if (!tempType.isErrorType()&&!tempType.isBoolType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
...
class IfElseStmtNode extends StmtNode {
    ...
    public void typeCheck(Type type) {
        ...
        myThenStmtList.typeCheck(type);
        myElseStmtList.typeCheck(type);
    }
...
class ReturnStmtNode extends StmtNode {
    ...
    public void typeCheck(Type type) {
        ...
        Type tempType = myExp.typeCheck();
        if (!tempType.equals(type) && !tempType.isErrorType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
                    "Bad return value");
        }
    }
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!lType.equals(rType) && !lType.isErrorType() && !rType.isErrorType()) {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (myExpList.getSize() == sym.getNumParams()) {
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (tempType.isErrorType()) {
            return new ErrorType();
        }
        else if (tempType.isIntType()) {
            return new IntType();
        }
        else {
            ErrMsg.fatal(lineNum(), charNum(),
...
class PlusNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        Type tempType1 = myExp1.typeCheck();
        Type tempType2 = myExp2.typeCheck();
        Type result = new IntType();

        if (tempType1.isErrorType() || tempType2.isErrorType()) {
            return new ErrorType();
        }

        if (!tempType1.isIntType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
                    "Arithmetic operator applied to non-numeric operand");
            result = new ErrorType();
        }

        if (!tempType2.isIntType()) {
...
class AndNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        Type tempType1 = myExp1.typeCheck();
        Type tempType2 = myExp2.typeCheck();
        Type result = new ErrorType();

        if (tempType1.isErrorType() || tempType2.isErrorType()) {
            return new ErrorType();
        }

        if (!tempType1.isBoolType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
                    "Logical operator applied to non-bool operand");
            tempType1 = new ErrorType();
        }
        if (!tempType2.isBoolType()) {
            ErrMsg.fatal(myExp2.lineNum(), myExp2.charNum(),
                    "Logical operator applied to non-bool operand");
            tempType1 = new ErrorType();
		}
...
class EqualsNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        Type tempType2 = myExp2.typeCheck();
        Type result = new ErrorType();
        boolean typeFlag = tempType1.equals(tempType2);
        ...
        if (tempType1.isVoidType() && typeFlag) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
                    "Equality operator applied to void functions");
        }
        else if (tempType1.isFnType() && typeFlag) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
                    "Equality operator applied to functions");
        }
        else if (tempType1.isStructType() && typeFlag) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
                    "Equality operator applied to struct variables");
        }
        else if (tempType1.isStructDefType() && typeFlag) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),
